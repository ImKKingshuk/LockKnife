...
_SIGNATURES: list[_CarveSignature] = [
    {"kind": "jpg", "header": b"\xff\xd8\xff", "footer": b"\xff\xd9", "extension": ".jpg", "max_size": 25 * 1024 * 1024},
    {"kind": "png", "header": b"\x89PNG\r\n\x1a\n", "footer": b"IEND\xaeB`\x82", "extension": ".png", "max_size": 25 * 1024 * 1024},
    {"kind": "pdf", "header": b"%PDF-", "footer": b"%%EOF", "extension": ".pdf", "max_size": 50 * 1024 * 1024},
    {"kind": "zip", "header": b"PK\x03\x04", "footer": b"PK\x05\x06", "extension": ".zip", "max_size": 50 * 1024 * 1024},
]


def carve_deleted_files(input_path: pathlib.Path, output_dir: pathlib.Path, *, source: str = "auto", max_matches: int = 50) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    scan_sources = _scan_sources(input_path, source=source)
    carved: list[dict[str, Any]] = []
    counter = 0
    for source_entry in scan_sources:
        batch, counter = _carve_from_blob(source_entry, output_dir=output_dir, max_matches=max_matches - len(carved), counter=counter)
        carved.extend(batch)
    ...
    return {
        ...
        "sources": [{key: value for key, value in item.items() if key != "blob"} for item in scan_sources],
        "carved": carved,
    }
...
def _carve_from_blob(source_entry: dict[str, Any], *, output_dir: pathlib.Path, max_matches: int, counter: int) -> tuple[list[dict[str, Any]], int]:
