...
from lockknife.modules._case_enrichment_payloads import api_discovery_payload, network_summary_payload, otx_payload, virustotal_payload



_NON_FATAL_ENRICHMENT_ERRORS: tuple[type[BaseException], ...] = (LockKnifeError, OSError, RuntimeError, TypeError, ValueError)
...
def _error_run_entry(workflow: str, artifact: dict[str, Any], error: str, *, input_path: str) -> dict[str, Any]:
    return _run_entry(
        workflow,
        artifact,
        {
...
def _pcap_runs(artifact: dict[str, Any], path: pathlib.Path) -> list[dict[str, Any]]:
    if path.suffix.lower() not in _PCAP_SUFFIXES and "network-capture" not in str(artifact.get("category") or ""):
        return []
    return [
        _run_entry("network.summarize", artifact, network_summary_payload(summarize_pcap(path), input_path=path)),
        _run_entry(
    ...
    *,
    ...
    for match in matches:
        ...
        if kind in {"domain", "ipv4", "url", "sha256"}:
            try:
                runs.append(_run_entry("intelligence.otx", artifact, otx_payload(indicator, indicator_reputation(indicator), input_paths=[str(path)])))
            except _NON_FATAL_ENRICHMENT_ERRORS as exc:
                runs.append(_error_run_entry("intelligence.otx", artifact, str(exc), input_path=str(path)))
        if kind == "sha256" and len(runs) < limit and _looks_like_sha256(indicator):
            try:
                runs.append(_run_entry("intelligence.virustotal", artifact, virustotal_payload(indicator, file_report(indicator), input_paths=[str(path)])))
            except _NON_FATAL_ENRICHMENT_ERRORS as exc:
                runs.append(_error_run_entry("intelligence.virustotal", artifact, str(exc), input_path=str(path)))
    return runs[:limit]
