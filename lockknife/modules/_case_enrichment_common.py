...
def _summarize_matches(matches: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    unique_iocs = {str(match.get("ioc") or "").strip() for match in matches if str(match.get("ioc") or "").strip()}
    confidences = [float(match.get("confidence") or 0.0) for match in matches if isinstance(match.get("confidence"), (int, float))]
    return {
        "match_count": len(matches),
        "unique_ioc_count": len(unique_iocs),
        "composite_count": sum(1 for match in matches if str(match.get("kind") or "").startswith("composite_")),
