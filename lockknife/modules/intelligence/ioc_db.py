...
def init_db(path: pathlib.Path) -> None:
    ...
    try:
        con.execute(
            "CREATE TABLE IF NOT EXISTS iocs (ioc TEXT PRIMARY KEY, kind TEXT, source TEXT, first_seen REAL)"
        )
        con.execute(
...
def add_iocs(path: pathlib.Path, records: list[IocRecord]) -> int:
    ...
    try:
        ...
        for r in records:
            cur.execute(
                "INSERT OR REPLACE INTO iocs(ioc, kind, source, first_seen) VALUES (?, ?, ?, ?)",
                (r.ioc, r.kind, r.source, float(r.first_seen)),
            )
            n += 1
        con.commit()
        return n
...
def list_iocs(path: pathlib.Path, limit: int = 200) -> list[IocRecord]:
    ...
    try:
        cur = con.cursor()
        cur.execute(
            "SELECT ioc, kind, source, first_seen FROM iocs ORDER BY first_seen DESC LIMIT ?",
            (limit,),
        )
        return [
            IocRecord(ioc=ioc, kind=kind, source=source, first_seen=float(first_seen))
            for ioc, kind, source, first_seen in cur.fetchall()
        ]
    finally:
        con.close()
    ...
    *,
    ...
    try:
        for feed in feeds:
            ...
            if (
                not force
                and state.get("last_success")
                and (now() - float(state["last_success"])) < min_refresh_seconds
            ):
                results.append(
                    {
                        "name": name,
            ...
            try:
                ...
                results.append(
                    {"name": name, "feed_type": feed_type, "status": "updated", "added": added}
                )
                total_added += added
...
def _feed_state(con: sqlite3.Connection, name: str) -> dict[str, Any]:
    row = con.execute(
        "SELECT feed_name, feed_type, last_success, last_error, last_count FROM ioc_feed_sync WHERE feed_name=?",
        (name,),
    ).fetchone()
    if row is None:
        return {}
...
def _load_feed_records(feed: dict[str, Any]) -> Iterable[IocRecord]:
    ...
    if feed_type == "stix_url":
        ...
        return [
            ...
            for item in load_stix_indicators_from_url(source)
        ]
    if feed_type == "taxii":
    ...
    if feed_type == "raw_url":
        source = str(feed["url"])
        raw = http_get(source, timeout_s=20.0, max_attempts=4, cache_ttl_s=10 * 60).decode(
            "utf-8", errors="ignore"
        )
        return [
            IocRecord(ioc=item.ioc, kind=item.kind, source=source, first_seen=now())
            for item in detect_iocs(raw, location=source)
        ]
...
def _load_otx_records(feed: dict[str, Any]) -> list[IocRecord]:
    ...
    payload = http_get_json(
        url, headers={"X-OTX-API-KEY": api_key}, timeout_s=20.0, max_attempts=4, cache_ttl_s=10 * 60
    )
