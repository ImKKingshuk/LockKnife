...
def handle(app: Any, action: str, params: dict[str, Any], *, cb: Any) -> dict[str, Any] | None:
    ...
    if action == "device.list":
        devices = app.devices.list_handles()
        payload = [
            {
                "serial": d.serial,
        ...
        return _ok(payload, f"{len(payload)} devices detected")

    if action == "device.info":
        serial = _require(params, "serial")
        info = app.devices.info(serial)
        return _ok(_asdict(info), f"Loaded device info for {serial}")

    if action == "device.connect":
        host = _require(params, "host")
