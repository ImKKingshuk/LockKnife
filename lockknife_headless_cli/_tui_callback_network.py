...
def handle(app: Any, action: str, params: dict[str, Any], *, cb: Any) -> dict[str, Any] | None:
    ...
    if action == "network.capture":
        ...
        output, _derived = _resolve_case_output(
            ...
            case_dir,
            area="evidence",
            filename=f"network_capture_{serial}.pcap",
        )
        if output is None:
        ...
        _register_case_output(
            ...
            metadata={"duration_s": duration, "iface": iface},
        )
        return _ok(dataclasses.asdict(result), f"Capture saved to {output}")

    if action == "network.summarize":
        ...
        if output is not None:
            ...
            _register_case_output(
                ...
                metadata={"pcap": str(path), **(payload.get("summary") or {})},
            )
            return _ok(payload, f"PCAP summary saved to {output}")
        return _ok(payload, "PCAP summary complete")

    if action == "network.api_discovery":
        path = pathlib.Path(_require(params, "path"))
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
            case_dir,
            area="derived",
            filename=f"network_api_discovery_{path.stem}.json",
        )
        payload = api_discovery_payload(
            extract_api_endpoints_from_pcap(path),
            input_path=path,
            case_dir=case_dir,
            output=output,
        )
        if output is not None:
            ...
            return _ok(payload, f"API discovery saved to {output}")
        return _ok(payload, "API discovery complete")
