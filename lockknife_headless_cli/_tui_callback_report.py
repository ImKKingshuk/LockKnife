...
def handle(app: Any, action: str, params: dict[str, Any], *, cb: Any) -> dict[str, Any] | None:
    ...
    case_chain_of_custody_report = cb.case_chain_of_custody_report
    ...
    build_chain_of_custody_payload = cb.build_chain_of_custody_payload
    generate_chain_of_custody = cb.generate_chain_of_custody
    sign_report_file = cb.sign_report_file
    ...
    if action == "report.generate":
        ...
        report_filename = {
            ...
            "pdf": f"{filename_prefix}_{case_id}.pdf",
        }.get(out_format, f"{filename_prefix}_{case_id}.html")
        ...
        if artifacts_path:
            artifacts = json.loads(pathlib.Path(artifacts_path).read_text(encoding="utf-8"))
        else:
            artifacts = inline_artifacts
            if not artifacts and report_case_dir is not None:
        ...
        if out_format == "json":
            ...
            _register_case_output(
                case_dir,
                path=actual_output,
                category=f"report-{pdf_result['format']}",
                source_command="report generate",
                input_paths=artifact_inputs,
                metadata={
            ...
            return _ok(result_payload, f"Report saved to {actual_output}")
        else:
            template_path = _template_path(template)
    ...
    if action == "report.chain_of_custody":
        case_dir = _path_param(params.get("case_dir"))
        case_id = _resolve_report_case_id(params, case_dir)
        examiner = _resolve_report_examiner(params, case_dir)
        out_format = str(params.get("format") or "text").lower()
        suffix = "html" if out_format == "html" else "txt"
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
            case_dir,
            area="reports",
            filename=f"chain_of_custody_{case_id}.{suffix}",
        )
        if output is None:
            raise ValueError("Either output or case_dir is required")
        evidence_paths = _csv_list(params.get("evidence"))
        notes = _opt(params.get("notes"))
        if case_dir is not None and not evidence_paths:
            payload = case_chain_of_custody_report(case_dir, notes=notes)
            evidence_items = case_chain_of_custody_items(case_dir)
        else:
            evidence_items = _custody_evidence_items(case_dir, evidence_paths)
    ...
    if action == "report.integrity":
        ...
        output, _derived = _resolve_case_output(
            ...
            case_dir,
            area="reports",
            filename=f"integrity_{manifest.case_id}.{suffix}",
        )
        if output is None:
            raise ValueError("Either output or case_dir is required")
        ...
        _register_case_output(
            ...
            metadata={"report_case_id": manifest.case_id, "format": out_format},
        )
        return _ok({"output": str(output), **report_payload}, f"Integrity report saved to {output}")
