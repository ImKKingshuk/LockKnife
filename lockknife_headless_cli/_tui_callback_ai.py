...
def handle(app: Any, action: str, params: dict[str, Any], *, cb: Any) -> dict[str, Any] | None:
    ...
    load_personal_data = cb.load_personal_data
    ...
    if action == "ai.anomaly_score":
        ...
        output, _derived = _resolve_case_output(
            ...
            case_dir,
            area="derived",
            filename=f"ai_anomaly_{_safe_name(input_path.stem)}.json",
        )
        rows = json.loads(input_path.read_text(encoding="utf-8"))
        payload = anomaly_payload(
            rows,
            feature_keys,
            anomaly_scores(rows, feature_keys),
    ...
    if action == "ai.predict_passwords":
        wordlist = pathlib.Path(_require(params, "wordlist"))
        personal_data_path = _path_param(params.get("personal_data"))
        count = int(params.get("count") or 50)
        min_len = int(params.get("min_len") or 6)
        max_len = int(params.get("max_len") or 12)
        seed_val = params.get("seed")
        seed = int(seed_val) if seed_val is not None and str(seed_val).strip() else None
        markov_order = int(params.get("markov_order") or 2)
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
            case_dir,
