...
def handle(app: Any, action: str, params: dict[str, Any], *, cb: Any) -> dict[str, Any] | None:
    ...
    if action == "security.audit":
        ...
        if output is not None:
            ...
            return _ok(findings, f"Audit findings saved to {output}")
        return _ok(findings, f"Audit findings: {len(findings)}")

    if action == "security.selinux":
        serial = _require(params, "serial")
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
    ...
    if action == "security.malware":
        rule = pathlib.Path(_require(params, "rule"))
        target = pathlib.Path(_require(params, "target"))
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
            case_dir,
            area="derived",
            filename=f"security_malware_{target.stem}.json",
        )
        matches = [dataclasses.asdict(m) for m in scan_with_yara(rule, target)]
        payload = {"engine": "yara", "matches": matches, "target": str(target)}
        if output is not None:
            ...
            _register_case_output(
                ...
                metadata={"engine": "yara", "pattern_count": 0, "match_count": len(matches)},
            )
            return _ok(payload, f"Malware scan saved to {output}")
        return _ok(payload, "Malware scan complete")

    if action == "security.network_scan":
        serial = _require(params, "serial")
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
        ...
        if output is not None:
            ...
            return _ok(payload, f"Network scan saved to {output}")
        return _ok(payload, "Network scan complete")

    if action == "security.bootloader":
        serial = _require(params, "serial")
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
        ...
        if output is not None:
            ...
            _register_case_output(
                ...
                device_serial=serial,
                metadata={"unlocked": payload.get("unlocked")},
            )
            return _ok(payload, f"Bootloader analysis saved to {output}")
        return _ok(payload, "Bootloader analysis complete")

    if action == "security.hardware":
        serial = _require(params, "serial")
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
        ...
        if output is not None:
            ...
            _register_case_output(
                ...
                device_serial=serial,
                metadata={"tee_present": payload.get("tee_present")},
            )
            return _ok(payload, f"Hardware security saved to {output}")
        return _ok(payload, "Hardware security complete")

    if action == "security.attack_surface":
        package = _opt(params.get("package"))
        serial = _opt(params.get("serial")) or _opt(getattr(app, "selected_device_serial", None))
        apk = _path_param(params.get("apk"))
        artifacts = _path_param(params.get("artifacts"))
        case_dir = _path_param(params.get("case_dir"))
        ...
        output, _derived = _resolve_case_output(
            ...
            case_dir,
            area="derived",
            filename=f"security_attack_surface_{_safe_name(str(name_hint))}.json",
        )
        payload = dict(report)
        ...
        if output is not None:
            ...
            _register_case_output(
                ...
                metadata={
                    ...
                    "live_probe_enabled": bool(probe_results_dict.get("attempted")),
                    "risk_level": risk_summary_dict.get("level"),
                },
            )
            return _ok(payload, f"Attack-surface assessment saved to {output}")
        return _ok(payload, "Attack-surface assessment complete")

    if action == "security.owasp":
        artifacts = pathlib.Path(_require(params, "artifacts"))
        case_dir = _path_param(params.get("case_dir"))
        output, _derived = _resolve_case_output(
            _path_param(params.get("output")),
            case_dir,
            area="derived",
            filename=f"security_owasp_{_safe_name(artifacts.stem)}.json",
        )
        payload = mastg_summary(json.loads(artifacts.read_text(encoding="utf-8")))
        if output is not None:
            write_json(output, payload)
            _register_case_output(
                case_dir,
            ...
            return _ok(payload, f"OWASP MASTG mapping saved to {output}")
        return _ok(payload, "OWASP MASTG mapping complete")
