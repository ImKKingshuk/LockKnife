...
#[pyfunction]
pub fn parse_ipv4_header_json(py: Python<'_>, packet_bytes: &[u8]) -> PyResult<String> {
                    ...
                    if qdcount > 0 {
                        if let Some((name, next_off)) = parse_dns_qname(dns, 12) {
                            if dns.len() >= next_off + 4 {
                                ...
                                qname =
                                    Some(json!({"name": name, "qtype": qtype, "qclass": qclass}));
                            }
                        }
                    }
                    out["dns"] = json!({
...
mod tests {
    ...
    fn test_ipv4_min_packet() {
        ...
        let out = pyo3::Python::attach(|py| parse_ipv4_header_json(py, &pkt).unwrap());
    ...
    fn test_ipv4_rejects_small() {
        init_python();
        let err = pyo3::Python::attach(|py| parse_ipv4_header_json(py, &[1, 2, 3]).unwrap_err());
