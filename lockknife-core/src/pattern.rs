...
#[pyfunction]
pub fn scan_patterns_json(py: Python<'_>, data: &[u8], patterns: Vec<String>) -> PyResult<String> {
    ...
    py.detach(move || {
        ...
        for (orig_i, raw) in special {
            if let Some(hex_pattern) = raw.strip_prefix("hex:") {
                let needle =
                    parse_hex_pattern(hex_pattern.trim()).map_err(PyValueError::new_err)?;
                ...
                let re =
                    Regex::new(re_pattern).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
mod tests {
    ...
    fn test_scan_literal_patterns() {
        let out = with_py(|py| scan_patterns_json(py, b"abcabc", vec!["abc".to_string()]).unwrap());
    ...
    fn test_scan_hex_pattern_with_wildcard() {
        let out = with_py(|py| {
            scan_patterns_json(py, b"\x41\x42\x43\x44", vec!["hex:41????44".to_string()]).unwrap()
        });
    ...
    fn test_scan_regex_pattern() {
        let out = with_py(|py| {
            scan_patterns_json(
                py,
                b"token=abc token=xyz",
                vec!["re:token=\\w+".to_string()],
            )
            .unwrap()
        });
    ...
    fn test_scan_empty_and_no_match() {
        let out = with_py(|py| {
            scan_patterns_json(py, b"abc", vec!["".to_string(), "zzz".to_string()]).unwrap()
        });
    ...
    fn test_scan_overlapping() {
        let out = with_py(|py| scan_patterns_json(py, b"aaaa", vec!["aa".to_string()]).unwrap());
    ...
    fn test_size_limit_rejects_oversized() {
        ...
        let out = with_py(|py| scan_patterns_json(py, b"x", vec!["x".to_string()]).unwrap());
