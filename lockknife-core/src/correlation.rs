...
#[pyfunction]
pub fn correlate_artifacts_json(py: Python<'_>, payloads: Vec<String>) -> PyResult<String> {
    py.detach(move || {
...
mod tests {
    ...
    fn test_correlate_empty_input() {
        init_python();
        let out = pyo3::Python::attach(|py| correlate_artifacts_json(py, vec![]).unwrap());
    ...
    fn test_correlate_malformed_json() {
        ...
        let err = pyo3::Python::attach(|py| {
            correlate_artifacts_json(py, vec!["{".to_string()]).unwrap_err()
        });
