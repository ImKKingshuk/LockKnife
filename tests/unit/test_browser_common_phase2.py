...
from lockknife.modules.extraction._browser_common import (
    _sh_quote,
    _table_columns,
    _try_root_pull_file,
)
...
class _Devices:
    def __init__(
        ...
        source_bytes: bytes,
        *,
        direct_pull: bool = False,
        cleanup_fails: bool = False,
        shell_fails: bool = False,
    ) -> None:
...
def test_browser_common_helpers_and_root_pull(tmp_path: pathlib.Path) -> None:
    ...
    assert (
        _try_root_pull_file(
            _Devices(b"abc", direct_pull=True),
            "SERIAL",
            "/data/data/file",
            direct_local,
            timeout_s=1.0,
    ...
    assert (
        _try_root_pull_file(devices, "SERIAL", "/data/data/file", fallback_local, timeout_s=1.0)
        is True
    )
    ...
    assert (
        _try_root_pull_file(
            _Devices(b"nope", shell_fails=True),
            "SERIAL",
            "/data/data/file",
            failed_local,
            timeout_s=1.0,
