...
def test_apk_network_intel_and_passkeys_case_registration(
    ...
    create_case_workspace(
        case_dir=case_dir, case_id="CASE-011", examiner="Examiner", title="Phase 3"
    )
    ...
    monkeypatch.setattr(
        apk_cli,
        "parse_apk_manifest",
        lambda *_a, **_k: {"package": "com.example.app", "permissions": []},
    )
    monkeypatch.setattr(
        apk_cli,
        "analyze_apk",
    ...
    pcap_artifact = register_case_artifact(
        case_dir=case_dir,
        path=pcap_path,
    ...
    monkeypatch.setattr(
        network_cli,
        "capture_pcap",
        lambda *_a, output_path, duration_s, iface, snaplen, **_k: (
            output_path.write_text("pcap", encoding="utf-8"),
            CaptureResult(
                serial="SER-1",
                remote_path="/sdcard/x.pcap",
    ...
    monkeypatch.setattr(
        network_cli, "summarize_pcap", lambda path: {"pcap": str(path), "summary": "ok"}
    )
    monkeypatch.setattr(
        network_cli,
        "extract_api_endpoints_from_pcap",
        lambda path: {"pcap": str(path), "endpoints": []},
    )

    result = runner.invoke(
        network_cli.network, ["capture", "-s", "SER-1", "--case-dir", str(case_dir)], obj=app
    )
    assert result.exit_code == 0, result.output
    result = runner.invoke(
        network_cli.network, ["analyze", str(pcap_path), "--case-dir", str(case_dir)]
    )
    assert result.exit_code == 0, result.output

    ioc_input = case_dir / "evidence" / "iocs.json"
    ioc_input.write_text(json.dumps([{"ioc": "x"}]), encoding="utf-8")
    ioc_parent = register_case_artifact(
        case_dir=case_dir, path=ioc_input, category="analysis-input", source_command="case register"
    )
    monkeypatch.setattr(intel_cli, "detect_iocs", lambda *_a, **_k: [_SmsRow("ioc")])

    result = runner.invoke(
        intel_cli.intel, ["ioc", "--input", str(ioc_input), "--case-dir", str(case_dir)]
    )
    assert result.exit_code == 0, result.output
    ...
    result = runner.invoke(
        crack_cli.crack, ["passkeys", "-s", "SER-1", "--case-dir", str(case_dir)], obj=app
    )
    assert result.exit_code == 0, result.output
    ...
    network_analysis = next(
        artifact for artifact in manifest.artifacts if artifact.category == "network-analysis"
    )
    assert network_analysis.parent_artifact_ids == [pcap_artifact.artifact_id]
    intel_ioc = next(
        artifact for artifact in manifest.artifacts if artifact.category == "intel-ioc"
    )
    assert intel_ioc.parent_artifact_ids == [ioc_parent.artifact_id]
    passkeys_manifest = next(
        artifact
        for artifact in manifest.artifacts
        if artifact.category == "crack-passkeys-manifest"
    )
    assert passkeys_manifest.parent_artifact_ids


def test_crack_credential_workflows_register_case_artifacts(
    monkeypatch, tmp_path: pathlib.Path
) -> None:
    ...
    case_dir = tmp_path / "case"
    create_case_workspace(
        case_dir=case_dir, case_id="CASE-012", examiner="Examiner", title="Phase 7"
    )
    ...
    assert (
        runner.invoke(crack_cli.crack, ["wifi", "--case-dir", str(case_dir)], obj=app).exit_code
        == 0
    )
    assert (
        runner.invoke(crack_cli.crack, ["gesture", "--case-dir", str(case_dir)], obj=app).exit_code
        == 0
    )
    assert (
        runner.invoke(crack_cli.crack, ["keystore", "--case-dir", str(case_dir)], obj=app).exit_code
        == 0
    )
    assert (
        runner.invoke(
            crack_cli.crack, ["pin-device", "--length", "6", "--case-dir", str(case_dir)], obj=app
        ).exit_code
        == 0
    )
...
def test_runtime_and_security_case_registration(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    case_dir = tmp_path / "case"
    create_case_workspace(
        case_dir=case_dir, case_id="CASE-012", examiner="Examiner", title="Runtime Security"
    )
    app = SimpleNamespace(devices=SimpleNamespace())
    ...
    target_artifact = register_case_artifact(
        case_dir=case_dir, path=target, category="evidence-file", source_command="case register"
    )
    mastg_input = case_dir / "derived" / "artifacts.json"
    mastg_input.write_text(json.dumps([{"id": "x"}]), encoding="utf-8")
    mastg_artifact = register_case_artifact(
        case_dir=case_dir,
        path=mastg_input,
    ...
    monkeypatch.setattr(
        ...
        "scan_network",
        lambda *_a, **_k: SimpleNamespace(
            dns=["192.0.2.4"], dns_cache=[], listening=[_Listener("tcp")], raw="ok"
        ),
    )
    ...
    security_malware = next(
        artifact for artifact in manifest.artifacts if artifact.category == "security-malware"
    )
    assert target_artifact.artifact_id in security_malware.parent_artifact_ids
    security_owasp = next(
        artifact for artifact in manifest.artifacts if artifact.category == "security-owasp"
    )
    assert security_owasp.parent_artifact_ids == [mastg_artifact.artifact_id]
    runtime_session = next(
        artifact for artifact in manifest.artifacts if artifact.category == "runtime-session"
    )
...
def test_ai_and_crypto_case_registration(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    case_dir = tmp_path / "case"
    create_case_workspace(
        case_dir=case_dir, case_id="CASE-013", examiner="Examiner", title="AI Crypto"
    )

    features = case_dir / "evidence" / "features.json"
    features.write_text(json.dumps([{"f1": 1, "label": 0}]), encoding="utf-8")
    features_artifact = register_case_artifact(
        case_dir=case_dir, path=features, category="analysis-input", source_command="case register"
    )

    corpus = case_dir / "evidence" / "corpus.txt"
    corpus.write_text("secret\npassword\n", encoding="utf-8")
    corpus_artifact = register_case_artifact(
        case_dir=case_dir, path=corpus, category="wordlist", source_command="case register"
    )

    wallet_db = case_dir / "evidence" / "wallet.db"
    wallet_db.write_text("wallet", encoding="utf-8")
    wallet_artifact = register_case_artifact(
        case_dir=case_dir, path=wallet_db, category="wallet-db", source_command="case register"
    )

    monkeypatch.setattr(
        ai_cli, "anomaly_scores", lambda *_a, **_k: [{"row": {"f1": 1}, "anomaly_score": 0.1}]
    )
    ...
    monkeypatch.setattr(
        ai_cli, "predict_classifier", lambda *_a, **_k: [{"row": {"f1": 1}, "prediction": 1}]
    )
    monkeypatch.setattr(
        ai_cli,
        "PasswordPredictor",
        SimpleNamespace(
            train_from_wordlist=lambda *_a, **_k: SimpleNamespace(
                generate=lambda **_kw: ["pw1", "pw2"]
            )
        ),
    )
    monkeypatch.setattr(
        crypto_cli, "extract_wallet_addresses_from_sqlite", lambda *_a, **_k: [_SmsRow("addr")]
    )
    monkeypatch.setattr(
        crypto_cli, "enrich_wallet_addresses", lambda *_a, **_k: [{"address": "addr", "lookup": {}}]
    )

    result = runner.invoke(
        ai_cli.ai,
        ["anomaly", "--input", str(features), "--feature", "f1", "--case-dir", str(case_dir)],
    )
    assert result.exit_code == 0, result.output
    ...
    result = runner.invoke(
        ai_cli.ai,
        ["predict-password", "--corpus", str(corpus), "--count", "2", "--case-dir", str(case_dir)],
    )
    assert result.exit_code == 0, result.output

    result = runner.invoke(
        crypto_cli.crypto_wallet,
        ["wallet", str(wallet_db), "--lookup", "--case-dir", str(case_dir)],
    )
    assert result.exit_code == 0, result.output
    ...
    assert {
        ...
        "ai-password-predictions",
        "crypto-wallet",
    } <= categories

    ai_anomaly = next(
        artifact for artifact in manifest.artifacts if artifact.category == "ai-anomaly"
    )
    assert ai_anomaly.parent_artifact_ids == [features_artifact.artifact_id]
    ai_model = next(
        artifact for artifact in manifest.artifacts if artifact.category == "ai-malware-model"
    )
    assert ai_model.parent_artifact_ids == [features_artifact.artifact_id]
    ai_classification = next(
        artifact
        for artifact in manifest.artifacts
        if artifact.category == "ai-malware-classification"
    )
    ...
    ai_passwords = next(
        artifact
        for artifact in manifest.artifacts
        if artifact.category == "ai-password-predictions"
    )
    assert ai_passwords.parent_artifact_ids == [corpus_artifact.artifact_id]
    wallet_output = next(
        artifact for artifact in manifest.artifacts if artifact.category == "crypto-wallet"
    )
