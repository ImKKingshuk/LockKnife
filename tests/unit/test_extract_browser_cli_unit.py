...
def _build_cli(tmp_path: pathlib.Path):
    ...
    cli = SimpleNamespace(
        _resolve_case_output=lambda output, case_dir, filename: ((case_dir / filename) if output is None and case_dir is not None else output, output is None and case_dir is not None),
...
def test_extract_browser_all_and_single_kind_routes(tmp_path: pathlib.Path) -> None:
    ...
    for args in [
        ["browser", "-s", "S", "--app", "firefox", "--kind", "all", "--format", "json", "--case-dir", str(case_dir)],
        ["browser", "-s", "S", "--app", "edge", "--kind", "bookmarks", "--format", "csv", "--output", str(tmp_path / "bookmarks.csv")],
        ["browser", "-s", "S", "--app", "edge", "--kind", "downloads", "--format", "csv", "--output", str(tmp_path / "downloads.csv")],
        ["browser", "-s", "S", "--app", "edge", "--kind", "cookies", "--format", "json", "--output", str(tmp_path / "cookies.json")],
        ["browser", "-s", "S", "--app", "edge", "--kind", "passwords", "--format", "csv", "--output", str(tmp_path / "passwords.csv")],
        ["browser", "-s", "S", "--app", "firefox", "--kind", "history", "--format", "json", "--case-dir", str(case_dir)],
        ["browser", "-s", "S", "--app", "firefox", "--kind", "bookmarks", "--format", "csv", "--output", str(tmp_path / "ff-bookmarks.csv")],
        ["browser", "-s", "S", "--app", "firefox", "--kind", "passwords", "--format", "json", "--output", str(tmp_path / "ff-passwords.json")],
        ["browser", "-s", "S", "--app", "firefox", "--kind", "downloads", "--format", "json", "--output", str(tmp_path / "ff-downloads.json")],
    ]:
    ...
    assert json.loads((tmp_path / "cookies.json").read_text(encoding="utf-8"))[0]["value"] == "cookies"
    assert json.loads((tmp_path / "ff-passwords.json").read_text(encoding="utf-8"))[0]["value"] == "ff-passwords"
...
def test_extract_browser_rejects_csv_for_all(tmp_path: pathlib.Path) -> None:
    ...
    result = runner.invoke(extract, ["browser", "-s", "S", "--app", "chrome", "--kind", "all", "--format", "csv"], obj=app)
