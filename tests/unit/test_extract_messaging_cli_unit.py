...
def test_extract_messaging_routes_messages_and_artifacts(tmp_path: pathlib.Path) -> None:
    ...
    for args in [
        ["messaging", "-s", "S", "--app", "whatsapp", "--mode", "messages", "--format", "json", "--case-dir", str(case_dir)],
        ["messaging", "-s", "S", "--app", "telegram", "--mode", "messages", "--format", "csv", "--output", str(tmp_path / "tg.csv")],
        ["messaging", "-s", "S", "--app", "signal", "--mode", "messages", "--format", "json", "--output", str(tmp_path / "sig.json")],
        ["messaging", "-s", "S", "--app", "whatsapp", "--mode", "artifacts", "--format", "json", "--output", str(tmp_path / "wa-artifacts.json")],
        ["messaging", "-s", "S", "--app", "telegram", "--mode", "artifacts", "--format", "json", "--output", str(tmp_path / "tg-artifacts.json")],
        ["messaging", "-s", "S", "--app", "signal", "--mode", "artifacts", "--format", "json", "--case-dir", str(case_dir)],
    ]:
        result = runner.invoke(extract, args, obj=app)
        assert result.exit_code == 0, result.output

    assert json.loads((case_dir / "messaging_whatsapp_messages.json").read_text(encoding="utf-8"))[0]["value"] == "wa-msg"
    assert json.loads((tmp_path / "sig.json").read_text(encoding="utf-8"))[0]["value"] == "sig-msg"
    assert json.loads((tmp_path / "wa-artifacts.json").read_text(encoding="utf-8"))["app"] == "whatsapp"
    assert json.loads((case_dir / "messaging_signal_artifacts.json").read_text(encoding="utf-8"))["files"] == ["sig.db"]
...
def test_extract_messaging_rejects_csv_for_artifacts(tmp_path: pathlib.Path) -> None:
    ...
    result = runner.invoke(extract, ["messaging", "-s", "S", "--app", "telegram", "--mode", "artifacts", "--format", "csv"], obj=app)
