...
def test_timeline_helpers_cover_timestamp_coercion_and_case_source_discovery(
    ...
    manifest = types.SimpleNamespace(
        artifacts=[
            types.SimpleNamespace(
                path=str(sms_path), category="extract-sms", artifact_id="artifact-1"
            )
        ]
    )
    ...
    assert (
        unique_sources(
            discovered, [TimelineSource(family="sms", path=str(sms_path), origin="explicit")]
        )
        == discovered
    )
...
def test_build_timeline_covers_multiple_artifact_families() -> None:
    events = build_timeline(
        ...
        browser={
            "downloads": [
                {"start_time_raw": 11644473600000000 + 3_000_000, "url": "https://example.com"}
            ]
        },
        ...
        app_usage={
            "packages": [{"last_time_used_ms": 8, "package": "pkg"}],
            "events": [{"time": 9, "class": "Main"}],
        },
        ...
        parsed_artifacts={
            "artifacts": [
                {
                    "artifact_family": "messaging",
                    "records": [{"timestamp_ms": 13, "text": "from parsed"}],
                }
            ]
        },
    )
