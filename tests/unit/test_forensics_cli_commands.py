...
def test_forensics_helper_functions(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    monkeypatch.setattr(
        forensics_cli,
        "case_output_path",
        lambda case_dir, area, filename: case_dir / area / filename,
    )
    monkeypatch.setattr(
        forensics_cli, "register_case_artifact", lambda **kwargs: captured.append(kwargs)
    )

    explicit, derived = forensics_cli._resolve_forensics_output(
        tmp_path / "x.json", None, area="derived", filename="a.json"
    )
    assert explicit == tmp_path / "x.json"
    assert derived is False

    none_path, none_derived = forensics_cli._resolve_forensics_output(
        None, None, area="derived", filename="a.json"
    )
    ...
    case_dir.mkdir()
    derived_path, derived = forensics_cli._resolve_forensics_output(
        None, case_dir, area="derived", filename="a.json"
    )
    assert derived is True
    assert derived_path == case_dir / "derived" / "a.json"

    forensics_cli._register_forensics_output(
    ...
    monkeypatch.setattr(
        forensics_cli,
        "find_case_artifact",
        lambda _case_dir, path: (
            SimpleNamespace(artifact_id=f"artifact:{path.name}")
            if path.name != "missing.json"
            else None
        ),
    )
    parent_ids = forensics_cli._parent_artifact_ids(
        case_dir,
        [str(tmp_path / "a.json"), str(tmp_path / "a.json"), str(tmp_path / "missing.json")],
    )
...
def test_forensics_cli_output_branches(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    monkeypatch.setattr(
        forensics_cli,
        "find_case_artifact",
        lambda *_a, **_k: SimpleNamespace(artifact_id="parent-1"),
    )
    ...
    monkeypatch.setattr(
        forensics_cli,
        "analyze_sqlite",
        lambda path: _Analysis(path=str(path), tables=[_Row("t1")], objects=[{"kind": "index"}]),
    )
    monkeypatch.setattr(
        forensics_cli,
        ...
        lambda **_k: {
            "event_count": 2,
            "sources": [{"path": str(tmp_path / "src.json")}],
            "events": [{"ts_ms": 1}],
        },
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        forensics_cli, "correlate_artifacts_json_blobs", lambda blobs: {"input_count": len(blobs)}
    )
    monkeypatch.setattr(
        forensics_cli,
        "recover_deleted_records",
        lambda _path: {"fragments": [{"text": "x"}], "summary": {"high_confidence_count": 1}},
    )
    monkeypatch.setattr(
        forensics_cli,
        "carve_deleted_files",
        lambda input_path, output_dir, **_k: {
            "carved_count": 1,
    ...
    _invoke(
        forensics_cli.forensics,
        ["parse", "--input-dir", str(sample_dir), "--case-dir", str(case_dir)],
    )
    _invoke(
        forensics_cli.forensics, ["import-aleapp", str(sample_dir), "--case-dir", str(case_dir)]
    )
    _invoke(
        forensics_cli.forensics, ["decode-protobuf", str(proto_path), "--case-dir", str(case_dir)]
    )
    _invoke(
        forensics_cli.forensics,
        [
    ...
    assert json.loads((case_dir / "derived" / "sqlite_sample.json").read_text(encoding="utf-8"))[
        "path"
    ].endswith("sample.sqlite")
    assert (
        json.loads((case_dir / "derived" / "timeline.json").read_text(encoding="utf-8"))[
            "event_count"
        ]
        == 2
    )
    assert (
        json.loads((case_dir / "derived" / "parsed_artifacts.json").read_text(encoding="utf-8"))[
            "summary"
        ]["artifact_count"]
        == 1
    )
    assert (
        json.loads((out_dir / "carve_sample.json").read_text(encoding="utf-8"))["carved_count"] == 1
    )
