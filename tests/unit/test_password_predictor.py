...
def test_password_predictor_generates(tmp_path: pathlib.Path) -> None:
    ...
    personal.write_text(
        json.dumps({"owner": "Casey", "device": {"model": "Pixel8"}}), encoding="utf-8"
    )
    out = model.generate(
        count=20, min_len=4, max_len=12, seed=0, personal_data=load_personal_data(personal)
    )
    assert isinstance(out, list)
    assert any(
        item.lower().startswith("casey") or item.lower().startswith("pixel8") for item in out
    )
