...
def test_extract_wallet_addresses_from_sqlite(tmp_path: pathlib.Path) -> None:
    sample = (
        "addr 0x52908400098527886E0F7030069857D2E4169EE7 and 1BoatSLRHtKNngkdXEeobR76b53LETtpyT"
    )
...
def test_lookup_wallet_address_success(monkeypatch) -> None:
    monkeypatch.setattr(
        wallet_mod, "http_get_json", lambda *_a, **_k: {"final_balance": 1, "n_tx": 2}
    )
...
def test_list_wallet_transactions(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        "http_get_json",
        lambda *_a, **_k: {
            "txrefs": [{"tx_hash": "h", "value": 1, "confirmations": 2, "received": "r"}]
        },
    )
