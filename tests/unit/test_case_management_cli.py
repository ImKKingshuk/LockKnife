...
def test_extract_case_dir_writes_default_evidence_and_registers(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    create_case_workspace(case_dir=case_dir, case_id="CASE-004", examiner="Examiner", title="Extraction")
...
def test_forensics_snapshot_case_dir_writes_default_evidence_and_registers(monkeypatch, tmp_path: pathlib.Path) -> None:
    from lockknife_headless_cli import forensics as forensics_cli
    from lockknife.core.case import create_case_workspace, load_case_manifest

    case_dir = tmp_path / "case"
    create_case_workspace(case_dir=case_dir, case_id="CASE-005", examiner="Examiner", title="Snapshot")
    ...
    snapshot = next(artifact for artifact in manifest.artifacts if artifact.category == "forensics-snapshot")
    meta = next(artifact for artifact in manifest.artifacts if artifact.category == "forensics-snapshot-meta")
...
def test_forensics_timeline_case_dir_links_registered_inputs(tmp_path: pathlib.Path) -> None:
    ...
    case_dir = tmp_path / "case"
    create_case_workspace(case_dir=case_dir, case_id="CASE-005B", examiner="Examiner", title="Timeline")
    ...
    sms_artifact = register_case_artifact(case_dir=case_dir, path=sms, category="extract-sms", source_command="extract sms")
    calls_artifact = register_case_artifact(case_dir=case_dir, path=calls, category="extract-call-logs", source_command="extract call_logs")
    ...
    manifest = load_case_manifest(case_dir)
    timeline = next(artifact for artifact in manifest.artifacts if artifact.category == "forensics-timeline")
...
def test_report_registration_links_to_existing_case_artifact(tmp_path: pathlib.Path) -> None:
    ...
    runner = CliRunner()
    case_dir = tmp_path / "case"
    runner.invoke(case_group, ["init", "--case-id", "CASE-006", "--examiner", "Examiner", "--title", "Reporting", "--output", str(case_dir)])
    ...
    assert result.exit_code == 0, result.output
    manifest = load_case_manifest(case_dir)
    parent_artifact = next(artifact for artifact in manifest.artifacts if artifact.category == "analysis-input")
    report_artifact = next(artifact for artifact in manifest.artifacts if artifact.category == "report-json")
...
def test_case_summary_command_outputs_text_and_json(tmp_path: pathlib.Path) -> None:
    ...
    runner = CliRunner()
    case_dir = tmp_path / "case"
    create_case_workspace(case_dir=case_dir, case_id="CASE-007", examiner="Examiner", title="Summary", target_serials=["SER-1"])

    evidence = case_dir / "evidence" / "sms.json"
    evidence.write_text("[]", encoding="utf-8")
    register_case_artifact(case_dir=case_dir, path=evidence, category="extract-sms", source_command="extract sms", device_serial="SER-1")
    ...
    text_result = runner.invoke(case_group, ["summary", "--case-dir", str(case_dir), "--format", "text"])
    ...
    json_result = runner.invoke(case_group, ["summary", "--case-dir", str(case_dir), "--format", "json"])
...
def test_case_graph_command_outputs_text_and_json(tmp_path: pathlib.Path) -> None:
    from lockknife_headless_cli.case import case_group
    from lockknife.core.case import case_lineage_graph, create_case_workspace, register_case_artifact
    ...
    root_artifact = register_case_artifact(case_dir=case_dir, path=root, category="evidence", source_command="extract sms", device_serial="SER-1")
    ...
    assert graph["edges"] == [{"parent_artifact_id": root_artifact.artifact_id, "child_artifact_id": child_artifact.artifact_id}]

    text_result = runner.invoke(case_group, ["graph", "--case-dir", str(case_dir), "--format", "text"])
    assert text_result.exit_code == 0, text_result.output
    ...
    json_result = runner.invoke(case_group, ["graph", "--case-dir", str(case_dir), "--format", "json"])
    assert json_result.exit_code == 0, json_result.output
...
def test_case_export_command_creates_bundle_with_optional_artifacts(tmp_path: pathlib.Path) -> None:
    ...
    runner = CliRunner()
    case_dir = tmp_path / "case"
    create_case_workspace(case_dir=case_dir, case_id="CASE-009", examiner="Examiner", title="Bundle")
    ...
    register_case_artifact(case_dir=case_dir, path=evidence_path, category="extract-sms", source_command="extract sms", device_serial="SER-1")
    ...
    with zipfile.ZipFile(output) as archive:
        ...
        export_payload = json.loads(archive.read("CASE-009/bundle/export_metadata.json").decode("utf-8"))
    ...
    with zipfile.ZipFile(filtered_output) as archive:
        ...
        filtered_payload = json.loads(archive.read("CASE-009/bundle/export_metadata.json").decode("utf-8"))
...
def test_case_artifact_query_commands(tmp_path: pathlib.Path) -> None:
    ...
    assert f"- {child_artifact.artifact_id} | forensics-timeline | derived/timeline.json" in artifacts_text.output
    assert "Search: query=timeline" in artifacts_text.output

    artifacts_json = runner.invoke(
        case_group,
        ["artifacts", "--case-dir", str(case_dir), "--metadata-contains", "sms", "--format", "json"],
    )
    ...
    artifact_text = runner.invoke(
        case_group,
        ["artifact", "--case-dir", str(case_dir), "--artifact-id", child_artifact.artifact_id, "--format", "text"],
    )
    assert artifact_text.exit_code == 0, artifact_text.output
    assert f"Artifact: {child_artifact.artifact_id} | forensics-timeline | derived/timeline.json" in artifact_text.output
    ...
    assert f"Artifact Lineage: {root_artifact.artifact_id} | extract-sms | evidence/sms.json" in lineage_text.output
    assert f"- {child_artifact.artifact_id} | forensics-timeline | derived/timeline.json" in lineage_text.output
...
def test_case_register_command_conflict_modes(tmp_path: pathlib.Path) -> None:
    ...
    runner = CliRunner()
    case_dir = tmp_path / "case"
    create_case_workspace(case_dir=case_dir, case_id="CASE-015", examiner="Examiner", title="Register CLI")
