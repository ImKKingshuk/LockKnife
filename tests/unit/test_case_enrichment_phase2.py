...
def test_case_enrichment_common_and_helper_functions(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    assert (
        _source("provider", mode="remote", description="desc", rate_limit_hint="slow")[
            "rate_limit_hint"
        ]
        == "slow"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: {"artifacts": [artifact, {"artifact_id": "A2"}]},
    )
    assert (
        len(
            _selected_artifacts(
    ...
    json_path.write_text(
        '{"package": "com.example", "manifest": {"package": "fallback"}}', encoding="utf-8"
    )
...
def test_case_enrichment_runs_and_summary(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    monkeypatch.setattr(
        runs_mod, "extract_api_endpoints_from_pcap", lambda _path: [{"url": "https://api.example"}]
    )
    monkeypatch.setattr(
        runs_mod,
        "network_summary_payload",
        lambda data, input_path: {"summary": data, "source_attribution": [{"provider": "pcap"}]},
    )
    monkeypatch.setattr(
        runs_mod,
        "api_discovery_payload",
        lambda data, input_path: {
            "summary": {"count": len(data)},
            "source_attribution": [{"provider": "api"}],
        },
    )
    monkeypatch.setattr(
        runs_mod, "indicator_reputation", lambda indicator: {"indicator": indicator}
    )
    monkeypatch.setattr(
        runs_mod, "file_report", lambda indicator: (_ for _ in ()).throw(RuntimeError("vt down"))
    )
    monkeypatch.setattr(
        runs_mod,
        "otx_payload",
        lambda indicator, data, input_paths: {
            "summary": data,
            "provider": indicator,
            "source_attribution": [{"provider": "otx"}],
        },
    )
    monkeypatch.setattr(
        runs_mod,
        "virustotal_payload",
        lambda indicator, data, input_paths: {"summary": data, "provider": indicator},
    )

    assert _run_entry("wf", artifact, {"ok": True})["workflow"] == "wf"
    assert (
        _error_run_entry("wf", artifact, "boom", input_path=str(path))["payload"]["error"] == "boom"
    )
    ...
    run_summary = summarize_case_enrichment_runs(
        [
            {"workflow": "a", "status": "ok", "provider": "otx"},
            {"workflow": "b", "status": "error", "provider": "vt"},
        ],
...
def test_run_case_enrichment_success_and_error_paths(monkeypatch, tmp_path: pathlib.Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda artifact, path: (
            [
                {
                    "workflow": "network.case",
                    "payload": {"source_attribution": [{"provider": "pcap"}]},
                }
            ]
            if path.suffix == ".pcap"
            else (_ for _ in ()).throw(RuntimeError("pcap boom"))
            if artifact["artifact_id"] == "a1"
            else []
        ),
    )
    monkeypatch.setattr(
        ...
        "detect_iocs",
        lambda raw: (
            [IocMatch(ioc="evil.example", kind="domain", location="loc", confidence=0.9)]
    ...
    monkeypatch.setattr(
        orch,
        "ioc_payload",
        lambda matches, input_path: {
            "matches": matches,
            "source_attribution": [{"provider": "ioc"}],
        },
    )
    monkeypatch.setattr(
        orch,
        "_reputation_runs",
        lambda artifact, path, matches, limit: [
            {"workflow": "intel.otx", "payload": {"source_attribution": [{"provider": "otx"}]}}
        ],
    )
    monkeypatch.setattr(
        orch, "correlate_cves_for_apk_package", lambda package: {"package": package}
    )
    monkeypatch.setattr(
        orch,
        "cve_payload",
        lambda package, data, input_paths: {
            "package": package,
            "source_attribution": [{"provider": "osv"}],
        },
    )
    monkeypatch.setattr(
        orch,
        "anomaly_scores",
        lambda rows, feature_keys: [{"index": 0, "score": 0.5}] if rows else [],
    )
    monkeypatch.setattr(
        orch,
        "anomaly_payload",
        lambda rows, feature_keys, scores, input_path: {
            "rows": len(rows),
            "source_attribution": [{"provider": "ai"}],
        },
    )
    ...
    assert {item["provider"] for item in out["provider_status"]} >= {
        "otx",
        "osv",
        "ai",
        "pcap",
