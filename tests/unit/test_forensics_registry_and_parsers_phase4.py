...
def test_forensics_parsers_cover_json_xml_and_text_inputs(tmp_path: pathlib.Path) -> None:
    ...
    usage.write_text(
        "<root><package package='pkg'/><event class='MainActivity'/></root>", encoding="utf-8"
    )
    wifi = tmp_path / "wifi_history.xml"
    wifi.write_text(
        "<root><Network SSID='Cafe'><string name='LastConnectedTime'>123</string></Network></root>",
        encoding="utf-8",
    )
    ...
    usage_json.write_text(
        json.dumps({"packages": [{"package": "pkg-json"}], "events": [{"class": "JsonActivity"}]}),
        encoding="utf-8",
    )
...
def test_registry_iterates_registered_artifacts_and_app_data(
    tmp_path: pathlib.Path, monkeypatch
) -> None:
    (tmp_path / "accounts.json").write_text(
        json.dumps({"accounts": [{"name": "alice"}]}), encoding="utf-8"
    )
    (tmp_path / "settings.json").write_text(
        json.dumps({"theme": "dark", "count": 2}), encoding="utf-8"
    )
    (tmp_path / "prefs.xml").write_text(
        "<map><string name='theme'>dark</string></map>", encoding="utf-8"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda path: {"source_file": str(path), "message_count": 1},
    )
    ...
    assert (
        registry_mod._records_from_payload({"items": [{"name": "alice"}]})[0]["_section"] == "items"
    )
