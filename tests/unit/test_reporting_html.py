...
from lockknife.modules.reporting.html_report import _artifact_preview_rows, render_html_report, write_html_report
...
def test_render_builtin_report_templates(tmp_path: pathlib.Path) -> None:
    ...
    create_case_workspace(case_dir=case_dir, case_id="CASE-HTML", examiner="Examiner", title="Phase 2 Report")
    evidence = case_dir / "evidence" / "sample.json"
    evidence.write_text('{"package": "com.example.app", "score": 9}', encoding="utf-8")
    register_case_artifact(case_dir=case_dir, path=evidence, category="apk-analyze", source_command="apk analyze")

    context = build_report_context(case_id=None, artifacts=[{"name": "apk", "count": 1}], case_dir=case_dir)
...
def test_write_html_report_alias_resolution_and_preview_rows(tmp_path: pathlib.Path) -> None:
    ...
    alias_dir.mkdir()
    (alias_dir / "executive_report.html").write_text("<p>{{ artifact_preview_rows[0].value }}</p>", encoding="utf-8")
