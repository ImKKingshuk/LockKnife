...
def test_tui_callback_case_aware_actions_route_outputs_into_case_dir(
    ...
    monkeypatch.setattr(
        cb_helpers, "register_case_artifact", lambda **kwargs: registered.append(kwargs)
    )
    monkeypatch.setattr(
        cb,
        "find_case_artifact",
    ...
    cb.create_case_workspace(
        case_dir=case_dir, case_id="CASE-001", examiner="Examiner", title="TUI"
    )
    ...
    artifacts_json.write_text(
        json.dumps({"manifest": {"package": "com.example.app"}}), encoding="utf-8"
    )
    ...
    registered_by_category = {
        entry["category"]: pathlib.Path(entry["path"]) for entry in registered
    }
    assert registered_by_category["forensics-timeline"] == case_dir / "derived" / "timeline.json"
    assert (
        registered_by_category["network-analysis"]
        == case_dir / "derived" / "network_analyze_capture.json"
    )
    assert (
        registered_by_category["security-scan"]
        == case_dir / "derived" / "security_scan_SERIAL.json"
    )
    assert (
        registered_by_category["security-attack-surface"]
        == case_dir / "derived" / "security_attack_surface_com_example_app.json"
    )
...
def test_tui_callback_passkeys_register_case_artifacts(
    ...
    case_dir = tmp_path / "case"
    cb.create_case_workspace(
        case_dir=case_dir, case_id="CASE-777", examiner="Examiner", title="TUI"
    )
...
def test_tui_callback_report_generation_can_summarize_case_context_on_demand(
    ...
    case_dir = tmp_path / "case"
    cb.create_case_workspace(
        case_dir=case_dir, case_id="CASE-010", examiner="Examiner", title="Case"
    )
    monkeypatch.setattr(
        cb,
...
def test_tui_callback_case_first_reporting_support_actions_write_into_case_dir(
    ...
    integrity = callback(
        "report.integrity", {"case_dir": str(case_dir), "output": "", "format": "json"}
    )
...
def test_tui_callback_additional_case_aware_actions_route_outputs_into_case_dir(
    ...
    monkeypatch.setattr(
        cb, "heap_dump", lambda *_a, **_k: json.dumps({"path": "/sdcard/lockknife.hprof"})
    )
    monkeypatch.setattr(
        cb, "detect_iocs", lambda *_a, **_k: [DummyIoc("192.0.2.4", "ipv4", "body")]
    )
    monkeypatch.setattr(
        cb, "anomaly_scores", lambda *_a, **_k: [{"row": {"x": 1}, "anomaly_score": 0.9}]
    )
    monkeypatch.setattr(
        cb,
        "PasswordPredictor",
    ...
    monkeypatch.setattr(
        cb, "extract_wallet_addresses_from_sqlite", lambda *_a, **_k: [DummyRow("addr")]
    )
    ...
    assert (
        registered_by_category["apk-permissions"]
        == case_dir / "derived" / "apk_permissions_sample.json"
    )
    assert (
        registered_by_category["runtime-memory-search"]
        == case_dir / "derived" / "runtime_memory_search_com_example_app.json"
    )
    assert (
        registered_by_category["runtime-heap-dump"]
        == case_dir / "derived" / "runtime_heap_dump_com_example_app.json"
    )
    ...
    assert (
        registered_by_category["ai-password-predictions"]
        == case_dir / "derived" / "ai_predict_password_words.json"
    )
    assert (
        registered_by_category["crypto-wallet"]
        == case_dir / "derived" / "crypto_wallet_wallets.json"
    )
...
def test_tui_callback_case_action_messages_match_tui_labels(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "output": str(
                kwargs.get("output")
                or (kwargs["case_dir"] / "derived" / "case_enrichment_CASE-001.json")
            ),
        },
    )
    ...
    assert (
        ...
        == "Artifact detail ready for artifact-0001"
    )
    assert (
        callback(
            "case.lineage",
        ...
        == "Artifact lineage ready for artifact-0001"
    )
    assert (
        callback(
            "case.export",
        ...
        == f"Export bundle saved to {bundle_path}"
    )
...
def test_tui_callback_case_job_actions_surface_job_history_and_rerun_context(
    ...
    monkeypatch.setattr(
        ...
        lambda case_dir, **kwargs: (
            rerun_params.append(kwargs) or {"case_dir": str(case_dir), "case_id": "CASE-001"}
        ),
    )
    ...
    assert (
        callback("case.jobs", {"case_dir": str(case_dir)})["message"]
        == f"Job history ready for {case_dir}"
    )
    assert (
        callback("case.job", {"case_dir": str(case_dir), "job_id": "job-0007"})["message"]
        == "Job detail ready for job-0007"
    )
    assert callback("case.resume_job", {"case_dir": str(case_dir), "job_id": "job-0007"})[
        "message"
    ] == (f"Case summary ready for {case_dir}")
    assert callback("case.retry_job", {"case_dir": str(case_dir), "job_id": "job-0007"})[
        "message"
    ] == (f"Case summary ready for {case_dir}")
    assert rerun_params == [
        {
